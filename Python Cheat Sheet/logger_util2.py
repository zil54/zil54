...
class SeleniumExecutionLogger:

	def __init__(self, outputDir):
		self.outputDir = outputDir

	def log_success(self, seleniumaction, output, filename):
		logging.basicConfig(filename=self.outputDir + filename, level=logging.DEBUG)
		logging.info("Log output: " + seleniumaction + "\n" + output)

	def log_failure(self, seleniumaction, output, filename):
		logging.basicConfig(filename=self.outputDir + filename)
		logging.error("Log output: " + seleniumaction + "\n" + output)

	def log_warning(self, seleniumaction, output, filename):
		logging.basicConfig(filename=self.outputDir + filename)
		logging.warning("Log output: " + seleniumaction + "\n" + output)
